...
class FakeGateway:
    ...
    def invoke(self,command,payload=None,request_id='x'):
        ...
        if command=='player.next': return {'api_version':'runtime.v1','request_id':request_id,'task':{'node_id':'LN01-N02'}}
...
class DevARuntimeApplicationTests(unittest.TestCase):
    ...
    def test_player_flow_uses_runtime_truth_and_platform_presentation(self):
        loaded=self.call('player.load_node',{'node_id':'LN01-N01'})['data']; self.assertEqual('LN01-N01',loaded['task']['node_id'])
        answer={'schema':'ANSWER_DTO_v1','task_type':loaded['task']['task_type'],'text':'Luke names Peter and John'}
        submitted=self.call('player.submit_answer',{'node_id':'LN01-N01','answer':answer})['data']
        self.assertEqual('D5/runtime',submitted['truth_owner']); self.assertEqual('CORRECT',submitted['status']); self.assertEqual('LN01-N02',submitted['next_node_id']); self.assertEqual('runtime feedback',submitted['feedback']); self.assertEqual('GOSPEL_PARALLELS',submitted['mastery_consequence'][0]['concept_id'])
    def test_hint_evidence_save_restore_and_next_delegate_runtime(self):
        ...
        self.assertEqual('D5/runtime',self.call('player.save_checkpoint',{'campaign_id':'LN','mission_id':'LN-01','node_id':'LN01-N01'})['data']['truth_owner'])
