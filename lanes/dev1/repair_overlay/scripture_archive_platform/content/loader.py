...
class CanonicalContentLoader:
    ...
    def list_campaigns(self)->list[dict[str,Any]]:
        ...
        for m in self._missions:
            g=grouped.setdefault(m['campaign_id'],{'campaign_id':m['campaign_id'],'title':{'LN':'Остання ніч','PA':'Дорога Павла'}.get(m['campaign_id'],m['campaign_id']),'mission_count':0,'machine_node_count':0});g['mission_count']+=1;g['machine_node_count']+=int(m.get('node_count') or 0)
    ...
    def next_node_id(self,node_id:str,outcome:str='correct')->str|None:
        ...
        match=re.fullmatch(r'(?:RESOLVED_NODE\s+)?([A-Z]{2,4}\d{2}-[NO]\d{2})',raw.strip());return match.group(1) if match else None
...
class TaskPresentationMapper:
    ...
    def _option_objects(values):
        ...
        for item in values or []:
            if isinstance(item,dict):value=item.get('value',item.get('id',item.get('label','')));label=item.get('label',value)
            else:value=label=str(item)
    ...
    def to_renderable(self,node:dict[str,Any],mission:dict[str,Any]|None=None)->dict[str,Any]:
        task_type=self.infer_task_type(node);ui=dict(node.get('ui_metadata') or {});payload=dict(node.get('response_contract') or node.get('task_contract') or node.get('task_payload') or {})
        options=self._option_objects(ui.get('options') or payload.get('options') or []);evidence_options=self._option_objects(ui.get('evidence_options') or payload.get('evidence_options') or [])
        if not evidence_options and task_type in {'EVIDENCE_SELECT','CLAIM_EVIDENCE'}:evidence_options=self._option_objects(payload.get('options') or payload.get('evidence') or node.get('required_evidence') or [])
        ...
        raw_items=ui.get('items') or payload.get('items') or payload.get('ordered_items') or (payload.get('options') if task_type=='ORDERING' else []);items=[]
        ...
        if task_type=='MATCHING':
            if raw_pairs and all(isinstance(x,dict) and 'passage' in x and 'claim' in x for x in raw_pairs):
                rights=[{'id':str(x['claim']),'label':str(x['claim'])} for x in raw_pairs];pairs=[{'left_id':str(x['passage']),'left_label':str(x['passage']),'right_options':rights} for x in raw_pairs]
            elif raw_pairs and all(isinstance(x,dict) and 'left' in x and 'right' in x for x in raw_pairs):
                rights=[{'id':str(x['right']),'label':str(x['right'])} for x in raw_pairs];pairs=[{'left_id':str(x['left']),'left_label':str(x['left']),'right_options':rights} for x in raw_pairs]
        steps=list(ui.get('steps') or payload.get('steps') or (node.get('grading') or {}).get('steps') or []);normalized_steps=[]
        ...
        if task_type=='OT_NT_LINK' and not relation_types and payload.get('relation_category'):relation_types=[{'id':str(payload['relation_category']),'label':str(payload['relation_category'])}]
