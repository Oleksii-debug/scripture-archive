...
def _entry(
    ...
    *,
    ...
    entry: dict[str, Any] = {
        "package_path": normalize_archive_path(package_path),
        "size_bytes": len(data),
...
def build_manifest(
    ...
    overlay_root = repo_root / "lanes" / "dev1" / "repair_overlay" / "frontend"
...
def verify_reader(reader: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for expected in entries:
        if not isinstance(expected, dict):
            ...
            continue
        package_path = normalize_archive_path(str(expected.get("package_path", "")))
        actual_name = index.get(package_path)
        row: dict[str, Any] = {"package_path": package_path, "ok": False}
        if not package_path or actual_name is None:
            errors.append(f"MISSING {package_path or '<empty>'}")
            row["error"] = "missing"
            checked.append(row)
            continue
        try:
