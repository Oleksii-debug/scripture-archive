...
1. **Fresh process/session** — create the active `Session` through `PlayerMemoryService.start_session`. This closes a previously persisted open session as `restart_recovery`, preserving an adjacent-session boundary.
...
7. **Scheduler** — when no explicit adjacent set is supplied, `Scheduler` derives it from the immediately previous retained session and blocks only `EXACT`. Validated variants/cross-context remain eligible.
