...
function clean(value,max){
  return typeof value==='string'&&value.length>0&&value.length<=max&&value===value.trim()&&!hasForbiddenControl(value);
}

export class DossierUI{
  constructor({invoke,announce,showView,isActiveView=()=>true,enabled=true}){
    this.invoke=invoke;
  ...
  mount({nav,main}){
    ...
    heading.tabIndex=-1;
    const explanation=element('p',{text:'Досьє є лише похідним представленням поточного runtime-owned evidence. Воно не додає біографію, хронологію, консенсус або зв’язки, яких немає у канонічних доказах.'});
    const safety=element('p',{className:'notice info',text:'Якщо поточна видима доказова область не встановлює твердження, досьє чесно показує, що cited support у current scope недоступний; “Not stated in cited text” використовується лише коли відсутність встановлена.'});
    ...
    const linear=element('pre',{id:'dossier-linear'});linear.tabIndex=0;linear.setAttribute('aria-labelledby','dossier-linear-heading');linear.textContent='No cited support available in current scope';
  ...
  isCurrent(token){
    return this.visible&&this.isActiveView()&&this.generation===token;
  }

  async load(){
    if(!this.enabled||this.inFlight!==null)return this.inFlight;
    let request;
    try{request=this.request();}catch(error){this.status.textContent=error.message;this.announce?.(error.message);return;}
    const token=++this.generation;
    this.inFlight=token;
    this.submit.disabled=true;
    this.status.textContent='Завантаження source-safe досьє…';
    try{
      ...
      this.status.textContent=`Помилка досьє: ${error?.message||'невідома помилка'}`;
  ...
  validate(data,request){
    ...
    if(typeof d.stated!=='boolean'||typeof d.status_text!=='string'||d.status_text.length>500)throw new Error('Некоректний dossier status');
    ...
    for(const row of d.rows){
      ...
      if(row.witness!==null&&row.witness!==undefined&&!clean(row.witness,160))throw new Error('Некоректний dossier witness');
    }
    return d;
  }
  ...
  render(d){
    ...
    if(!d.rows.length){const tr=document.createElement('tr');const td=element('td',{text:d.status_text});td.colSpan=4;tr.append(td);this.tbody.append(tr);}
