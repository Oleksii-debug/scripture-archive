...
def project_cross_testament(
    ...
    *,
    ...
    No link is synthesized from topical similarity, shared entities, claims, or
    proximity. Relation/passages/witnesses are validated through the current
    canonical shared provenance boundary. Missing, hidden, contradictory, or
    malformed support fails closed rather than being normalized or harmonized.
    """
    ...
    if unknown_unlocks:
        raise ValueError(
            f"Unlocked evidence is not registered: {sorted(unknown_unlocks)!r}"
        )
    ...
    for relation_id in sorted(runtime.relations):
            ...
            sorted(
                {
                    item.evidence_id
                    for passage_id in ordered_ids
                    for item in visible_provenance.get(passage_id, ())
                }
            )
        )
        if not support_ids:
        ...
        visible_passage_ids = visible_relation_passage_ids(
            runtime,
            relation,
            support_ids,
            visible_evidence_ids=visible_evidence_ids,
        )
        if tuple(dict.fromkeys(visible_passage_ids)) != ordered_ids:
            continue
        ...
        if relation.witness is not None:
            ...
            if relation_witness != relation.witness:
                # The explicit relation itself may still be source-backed when its
                # optional witness label has no supporting witness metadata. Do not
                # publish that unproven label. If support *does* declare witness
                # metadata and it does not prove the relation witness, fail closed
                # instead of hiding a contradiction.
                has_support_witness = any(
                    runtime.evidence[evidence_id].witness is not None
                    or any(
        ...
        ot_ref, nt_ref = (
            (left, right) if left_testament == "OT" else (right, left)
        )
        ...
        links.append(
            CrossTestamentLink(
                ...
                relation_witness=relation_witness,
                ot=_endpoint(
                    ot_ref,
                    "OT",
...
def _endpoint(
    ...
    witnesses = {item.witness for item in provenance if item.witness is not None}
    endpoint_witness = passage.witness
    if endpoint_witness is None and len(witnesses) == 1:
        endpoint_witness = next(iter(witnesses))
