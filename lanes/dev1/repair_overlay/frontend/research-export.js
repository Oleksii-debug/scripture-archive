...
export class ResearchExportUI{
  ...
  mount({nav,main}){
    ...
    const safety=element('p',{className:'notice info',text:'HTML-експорт тут показується як звичайний текст і не виконується як DOM. Експортер не виводить дати, consensus або міжсвідкові твердження самостійно.'});

    const controls=element('div',{className:'action-row'});
    const formatLabel=element('label',{text:'Формат'});
    formatLabel.htmlFor='research-export-format';
    const format=document.createElement('select');
    format.id='research-export-format';
    [['json','JSON'],['markdown','Markdown'],['html','HTML']].forEach(([value,label])=>{
    ...
    const refresh=element('button',{id:'research-export-refresh',text:'Оновити експорт'});
    refresh.type='button';
    ...
    const status=element('div',{id:'research-export-status',className:'sr-status'});
    status.setAttribute('role','status');
    status.setAttribute('aria-live','polite');
    status.setAttribute('aria-atomic','true');

    const outputLabel=element('h3',{id:'research-export-output-heading',text:'Текст експорту'});
    const output=element('pre',{id:'research-export-output'});
    output.tabIndex=0;
    output.setAttribute('aria-labelledby','research-export-output-heading');
    output.textContent='Оберіть «Оновити експорт», щоб прочитати поточний unlocked-only snapshot.';

    const back=element('button',{id:'research-export-back',text:'Повернутися до дослідження'});
    back.type='button';
    back.onclick=()=>this.showView('research','research-heading');

    section.append(eyebrow,heading,scope,safety,controls,counts,status,outputLabel,output,back);
    main.append(section);
  ...
  async open(){
    if(!this.enabled){
      this.announce?.('Експорт недоступний без канонічного runtime');
      return;
    }
  ...
  async refresh(){
    ...
    const run=(async()=>{
      try{
        ...
        this.exportData=this.validate(data);
        this.render();
        const c=this.exportData.counts;
        this.counts.textContent=`Claims: ${c.claims}; evidence: ${c.evidence}; relations: ${c.relations}. Scope: unlocked_only.`;
        this.status.textContent='Експорт оновлено.';
        this.announce?.('Експорт дослідження оновлено');
      }catch(error){
        if(serial!==this.requestSerial)return;
        this.exportData=null;
        this.output.textContent='';
        this.counts.textContent='Експорт недоступний.';
        this.status.textContent=`Помилка експорту: ${error?.message||'невідома помилка'}`;
        this.announce?.(this.status.textContent);
      }finally{
        if(serial===this.requestSerial)this.refreshButton.disabled=false;
      }
    })();
    this.inFlight=run;
  ...
  validate(data){
    ...
    for(const key of ['claims','evidence','relations']){
      if(!Number.isInteger(value.counts[key])||value.counts[key]<0)throw new Error(`Некоректний count: ${key}`);
    }
    for(const key of FORMAT_KEYS){
      if(typeof value[key]!=='string')throw new Error(`Research export не містить ${key}`);
    }
    return value;
  }
  ...
  render(){
    ...
    const key=FORMAT_KEYS.has(this.format.value)?this.format.value:'json';
    this.output.textContent=this.exportData[key];
  }
}
