...
Set-Location $Repo
$Venv = Join-Path $Repo ".venv-r06-dev1-build"
...
& "$Venv\Scripts\pyinstaller.exe" --noconfirm --clean --onefile --windowed `
  --name ScriptureArchive-R06-DEV1 `
  --collect-all webview `
...
$Out = Join-Path $Dist "ScriptureArchive-R06-DEV1.exe"
...
"SHA256=$hash" | Set-Content -Encoding utf8 (Join-Path $Dist "ScriptureArchive-R06-DEV1.sha256.txt")
"SIZE=$size" | Add-Content -Encoding utf8 (Join-Path $Dist "ScriptureArchive-R06-DEV1.sha256.txt")
