...
class PersistenceSecurityTests(unittest.TestCase):
    ...
    def test_migration_creates_pre_migration_backup(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); (root/"state.json").write_text(json.dumps({"schema_version":1,"profile":{},"settings":{},"sessions":[],"history":{},"mastery":{},"review_queue":[],"constructor_drafts":{},"keymap":{}}),encoding="utf-8")
            state=PersistenceStore(root).load(); self.assertEqual(state["schema_version"],2); self.assertTrue((root/"backups"/"state.pre_migration_v1.json").exists()); self.assertIn("accessibility_state",state)
