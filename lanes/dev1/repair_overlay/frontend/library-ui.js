...
function hideLibraryWhenBaseViewOpens() {
  ...
  if (baseViews().some(view => !view.classList.contains('hidden'))) {
...
function renderCatalog(data) {
  ...
  if (data.bundled_full_bible_text === false) {
    notice.textContent = 'Повний біблійний текст не входить до цього пакета. Пошук охоплює лише player-visible canonical metadata та source references, які вже надає CanonicalContentLoader.';
  } else {
    notice.textContent = 'Статус повного біблійного тексту не підтверджено цим catalog response.';
  }
...
async function runSearch(event) {
  ...
  status('Пошук у видимому canonical індексі…');
  try {
    const data = await api('library.search', payload);
    ...
    byId('library-results-heading').focus();
  } catch (error) {
    byId('library-results').replaceChildren();
...
function buildSurface() {
  ...
  const submit = element('button', 'Шукати', {type: 'submit'});
