...
class DevARuntimeApplicationTests(unittest.TestCase):
    ...
    def test_hint_evidence_next_progress_mastery_save_restore_delegate_runtime(self):
        ...
        nxt=self.call('player.next',{'node_id':'LN01-N01'})['data']; self.assertEqual('LN01-N02',nxt['task']['node_id']); self.assertEqual('D5/runtime',nxt['truth_owner']); self.assertFalse(any(command=='player.navigate_branch' for command,_,_ in self.gateway.calls))
