...
class FakeContent:
    ...
    def get(self, node_id): return self.tasks[node_id]


class FakeLoader:
    def __init__(self, missions, mission_for_node): self.missions = missions; self.by_node = mission_for_node
    def list_campaigns(self): return [{"campaign_id": "LN"}]
    def list_missions(self, campaign_id): return list(self.missions) if campaign_id == "LN" else []
    def mission_for_node(self, node_id): return dict(self.by_node[node_id])
...
class PackagedDailyCaseProjectionTests(unittest.TestCase):
    def setUp(self):
        audited = {"mission_id":"LN-01","campaign_id":"LN","entry_node":"LN01-N01","canonical_status":"AUTHOR_COMPLETE / DEVELOPER_SOURCE_AUDITED / INDEPENDENT_AUDIT_PENDING"}
        authored_only = {"mission_id":"LN-02","campaign_id":"LN","entry_node":"LN02-N01","canonical_status":"AUTHOR_COMPLETE"}
        auditor_only = {"mission_id":"LN-03","campaign_id":"LN","entry_node":"LN03-N01","canonical_status":"AUDITOR_ACCEPTED"}
        self.missions = [audited, authored_only, auditor_only]
        tasks = {
            node_id: SimpleNamespace(raw={"node_id":node_id,"task_family":family}, task_type="SHORT_TEXT", mastery_domains=(concept,))
            for node_id, family, concept in [
                ("LN01-N01","entry","C-ENTRY"),("LN01-N02","review","C-REVIEW"),
                ("LN02-N01","authored","C-AUTHORED"),("LN03-N01","auditor","C-AUDITOR")
            ]
        }
        self.runtime = FakeRuntime(tasks)
        self.runtime.memory.review_queue.append(ReviewQueueItem(
            queue_id="review-1", concept_id="C-REVIEW", node_id="LN01-N02",
            due_at=FIXED_NOW - timedelta(days=1), priority=5,
            relation=RetrievalRelation.VARIANT, reason="runtime-owned review",
        ))
        self.loader = FakeLoader(self.missions, {
            "LN01-N01":audited,"LN01-N02":audited,"LN02-N01":authored_only,"LN03-N01":auditor_only,
        })

    def project(self):
        return DailyCaseProjection(self.runtime, self.loader, now_provider=lambda: FIXED_NOW).response()
    ...
    def test_projection_is_read_only_and_does_not_leak_task_truth(self):
        ...
        self.assertEqual({"LN01-N01","LN01-N02","LN02-N01","LN03-N01"}, set(self.runtime.provenance_checked))

    def test_naive_now_provider_fails_closed(self):
        projection = DailyCaseProjection(self.runtime, self.loader, now_provider=lambda: datetime(2026, 9, 12))
        with self.assertRaises(ValueError): projection.response()
...
class PackagedDailyCaseBoundaryTests(unittest.TestCase):
    def test_application_allowlist_requires_empty_payload(self):
        ...
        app = object.__new__(PlatformApplication); app.player_gateway = gateway
        self.assertIs(response, app._dispatch("player.get_daily_case", {}))
        with self.assertRaises(ValueError): app._dispatch("player.get_daily_case", {"limit": 50})

    def test_application_and_gateway_fail_closed_without_canonical_runtime(self):
        app = object.__new__(PlatformApplication); app.player_gateway = None
        with self.assertRaises(ValueError): app._dispatch("player.get_daily_case", {})
        gateway = RuntimeBackedPlayerGateway(lambda request: request)
        with self.assertRaises(RuntimeGatewayError): gateway.get_daily_case()
    ...
    def test_composed_ui_keeps_daily_case_read_only_and_safe(self):
        ...
        for forbidden_command in ("player.submit_answer","player.request_hint","player.navigate_branch","player.save_checkpoint","authoring."):
