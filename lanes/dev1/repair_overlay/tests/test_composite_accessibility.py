...
class CompositeAccessibilityTests(unittest.TestCase):
    ...
    def test_nested_drag_only_child_fails_and_linear_report_matches(self) -> None:
        task = self._render([{"step_id":"s1","task_type":"SHORT_TEXT","label":"Arrange","prompt":"Arrange the evidence.","drag_only":True,"accessibility":self._child_accessibility()}])
        inspection = task["accessibility"]["inspection"]
        self.assertFalse(inspection["passed"])
        match = [item for item in inspection["findings"] if item["code"] == "A11Y_EXPLICIT_VISUAL_OR_POINTER_DEPENDENCY" and item["path"] == "task.steps[0].drag_only"]
    ...
    def test_nested_media_without_text_equivalent_fails(self) -> None:
        task = self._render([{"step_id":"s1","task_type":"SHORT_TEXT","label":"Inspect map","prompt":"Describe the relevant location.","accessibility":self._child_accessibility(),"visual":{"media_slot":"map://case-1"}}])
        inspection = task["accessibility"]["inspection"]
        self.assertFalse(inspection["passed"])
    ...
    def test_keyboard_complete_nested_child_passes(self) -> None:
        task = self._render([{"step_id":"s1","task_type":"SHORT_TEXT","label":"Explain","prompt":"State what the cited text says.","accessibility":self._child_accessibility()}])
    ...
    def test_nested_single_choice_fails_packaged_renderer_answer_parity(self) -> None:
        task = self._render([{"step_id":"s1","task_type":"SINGLE_CHOICE","label":"Choose","prompt":"Choose the supported answer.","options":[{"id":"a","label":"Alpha"},{"id":"b","label":"Beta"}],"accessibility":self._child_accessibility()}])
        inspection = task["accessibility"]["inspection"]
        self.assertFalse(inspection["passed"])
    ...
    def test_nested_ordering_fails_packaged_renderer_answer_parity(self) -> None:
        task = self._render([{"step_id":"s1","task_type":"ORDERING","label":"Order","prompt":"Put the items in order.","items":[{"id":"first","label":"First"},{"id":"second","label":"Second"}],"accessibility":self._child_accessibility()}])
        inspection = task["accessibility"]["inspection"]
        self.assertFalse(inspection["passed"])
        self.assertIn(("A11Y_COMPOSITE_CHILD_RENDERER_PARITY_MISMATCH","task.steps[0].task_type"),{(item["code"], item["path"]) for item in inspection["findings"]})
    ...
    def test_grading_only_step_cannot_be_promoted_to_accessibility_contract(self) -> None:
        ...
        self.assertNotIn("grading-only answer truth", repr(task))

    def test_incomplete_nested_child_still_exposes_explicit_hazard(self) -> None:
        task = self._render([{"step_id":"s1","label":"Arrange","prompt":"Arrange the evidence.","visual_only":True}])
        inspection = task["accessibility"]["inspection"]
