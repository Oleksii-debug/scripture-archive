...
`SpeechRequest` carries exact `text`, `provider_id`, `voice_id`, optional `model`, `response_format`, optional `language`, presentation-only `purpose`, generation `speed`, optional provider-supported `instructions`, and a `private_text` privacy flag.
...
`SpeechService` validates the request, resolves a provider, computes a deterministic cache key, reuses cached output when available, otherwise calls the provider, atomically writes generated audio, and returns artifact metadata/path without changing source truth.
...
Current official endpoint: `POST https://api.openai.com/v1/audio/speech`
...
The adapter supports response formats accepted by the current OpenAI speech endpoint. The default model is configurable and currently defaults to `gpt-4o-mini-tts`. Provider-specific instructions are rejected when the selected model does not support them.

The built-in OpenAI adapter pins credential-bearing requests to the official `api.openai.com/v1/audio/speech` host/path. A different proxy or vendor must be implemented as a separate explicit provider adapter rather than silently redirecting this credential.
...
Current official endpoint: `POST https://api.elevenlabs.io/v1/text-to-speech/{voice_id}`

Credential reference: `ELEVENLABS_API_KEY`
...
OpenAI-style `instructions` are not silently discarded by this adapter; unsupported controls fail closed. The shared `speed` value is forwarded as ElevenLabs `voice_settings.speed` and is constrained to ElevenLabs' documented `0.7–1.2` range. `language_code` is forwarded only for models that support it; `eleven_multilingual_v2` rejects an explicit language code rather than relying on the provider to ignore it.

The built-in ElevenLabs adapter similarly pins credential-bearing requests to the official `api.elevenlabs.io/v1/text-to-speech` host/path.
...
This matters for personal notes, private research annotations, imported user text, and future connected/private sources. A future UI must state clearly when a network provider will receive text. The application must remain usable with TTS disabled.

Secrets are referenced only by environment-variable name; do not participate in cache keys; do not appear in sanitized provider errors; and must never be exported in content packs, research exports, state files, logs, or release packages.
...
Long-term release behavior should support a configurable cache location/size limit, clear-cache command, offline replay of cached speech, an optional OS/local speech provider as a no-network fallback, and explicit indication when requested audio is unavailable offline. No external API call is required for core gameplay.

## 7. Retry and failure policy

Network adapters use HTTPS-only credential-free endpoints, bounded timeouts, and retries only for transient network errors, HTTP 429, and HTTP 5xx. Retry is bounded exponential backoff with jitter. Authentication/validation failures are not retried as transient, and provider response bodies are not included in runtime exceptions.

A TTS failure is a presentation error. It does not fail grading, change mastery, or invalidate canonical text.

## 8. Accessibility / keyboard contract for UI convergence

The eventual packaged UI must provide keyboard-reachable controls for Speak/Play, Pause/Resume when supported, Stop, Replay, speech rate, provider, voice, and automatic-reading preferences.
...
Speech supports pedagogy rather than replacing it. Permitted targets include exact cited Scripture/passages, task prompts, substantive feedback, progressive hints, retrieval/review prompts, and research summaries that are already authorized visible text.

Narration must preserve `EXACT`, `VARIANT`, `PASSAGE_REVISIT`, `CROSS_CONTEXT`, and `SYNTHESIS` distinctions.
...
Foundation tests use fake HTTP/provider boundaries and make no paid network calls. Required regression coverage includes registry duplicate prevention, exact input preservation, deterministic cache reuse, no secret in cache identity, missing-credential fail-closed behavior, private network text blocked by default, correct OpenAI/ElevenLabs HTTP boundaries, output-format consistency, provider-specific speed/language validation, official-host pinning, unsupported provider controls fail-closed, transient retry behavior, sanitized terminal errors, and HTTPS-only endpoints.
