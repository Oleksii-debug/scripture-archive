...
class PA02StructuredMaterializationTests(unittest.TestCase):
    ...
    def test_registry_preserves_exact_validated_subject_catalog(self):
        bundle = load_r05_evidence_registry(REPO_ROOT)
        ...
        self.assertEqual(
            {
                ...
                "person:saul-paul",
                "person:ananias",
                "theme:companion-witness-boundary",
                "theme:commission-provenance",
            },
            set(bundle.subject_catalog),
        )
        encounter = bundle.subject_catalog["event:damascus-road"]
        self.assertEqual("event:damascus-road", encounter.subject_id)
        self.assertEqual("EVENT", encounter.kind)
        self.assertEqual("Damascus-road encounter", encounter.display_name)
        paul = bundle.subject_catalog["person:saul-paul"]
        self.assertEqual("PERSON", paul.kind)
        self.assertEqual("Saul / Paul", paul.display_name)

        for record in bundle.runtime.evidence.values():
            for subject_id in record.entity_ids:
                self.assertIn(subject_id, bundle.subject_catalog)
