...
def derive_answer_dto(node: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if ctype in {"SINGLE_CHOICE", "COMBOBOX_SELECT", "PARALLEL_WITNESS_COMPARE"}:
    ...
    elif ctype in {"SHORT_TEXT", "ARGUMENT"}:
        ...
        if isinstance(order, str) and "→" in order:
            order = [part.strip() for part in order.split("→")]
        dto = {"items": _listify(order)}
    elif ctype == "MATCHING":
        pairs = grading.get("accepted_pairs") or payload.get("pairs") or accepted
        if isinstance(pairs, Mapping):
            entries = [{"left": str(k), "right": str(v)} for k, v in pairs.items()]
        elif isinstance(pairs, list):
            entries = [{"left": str(p["left"]), "right": str(p["right"])} for p in pairs if isinstance(p, Mapping)]
        else:
            raise ValidationError("MATCHING content lacks structured pairs; content repair required")
        dto = {"pairs": entries}
    elif ctype == "EVIDENCE_SELECT":
        ids = grading.get("required_evidence_ids") or payload.get("correct_evidence")
        if not ids and isinstance(accepted, list):
            ids = accepted
        if not ids:
            ids = node.get("required_evidence")
        dto = {"evidence_ids": _listify(ids)}
    elif ctype == "CLAIM_EVIDENCE":
        if isinstance(accepted, Mapping):
            claim = accepted.get("claim") or accepted.get("text")
            ids = accepted.get("evidence_ids") or accepted.get("evidence_id")
        else:
            claim, ids = accepted, None
        ...
        if not isinstance(accepted, Mapping):
            raise ValidationError("SPEAKER_RECIPIENT accepted_answer must be object")
