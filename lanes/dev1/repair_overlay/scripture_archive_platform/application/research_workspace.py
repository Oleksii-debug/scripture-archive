...
class ResearchWorkspaceService:
    ...
    def _target(cls, value: Any) -> dict[str, Any]:
        ...
        kind = cls._identifier(value.get("kind"), "target.kind")
    ...
    def _load(self, key: str) -> dict[str, dict[str, Any]]:
        return self._records(self.store.get_json("research_workspace", key, {}) or {})
    ...
    def list_bookmarks(self, query: Any = None) -> list[dict[str, Any]]:
        ...
        return sorted(rows, key=lambda row: (str(row.get("title", "")).casefold(), str(row.get("bookmark_id", ""))))

    def upsert_bookmark(self, payload: Any) -> dict[str, Any]:
        if not isinstance(payload, dict):
            raise ValueError("bookmark must be object")
        unknown = set(payload) - {"bookmark_id", "title", "target", "tags"}
        if unknown:
    ...
    def list_notes(self, query: Any = None) -> list[dict[str, Any]]:
        ...
        return sorted(rows, key=lambda row: (str(row.get("title", "")).casefold(), str(row.get("note_id", ""))))

    def upsert_note(self, payload: Any) -> dict[str, Any]:
        if not isinstance(payload, dict):
            raise ValueError("note must be object")
        unknown = set(payload) - {"note_id", "title", "body", "target", "tags"}
        if unknown:
        ...
        target = self._target(payload.get("target")) if payload.get("target") is not None else None
        record = {
            "schema": NOTE_SCHEMA,
            "note_id": note_id,
            "title": self._plain_text(payload.get("title"), "title", max_len=200),
