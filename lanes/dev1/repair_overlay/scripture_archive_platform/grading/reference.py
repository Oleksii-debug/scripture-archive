...
def _norm(value:Any)->str:
    s=unicodedata.normalize('NFKC',str(value or '')).casefold();s=re.sub(r'[^\w]+',' ',s,flags=re.UNICODE);return ' '.join(s.split())

def _variants(node):
    ans=[];primary=node.get('accepted_answer')
...
class ReferenceGrader:
    """Safe DEV1 compatibility grader. DEV5 replaces this through GraderPort for production truth."""
    def grade(self,node:dict[str,Any],renderable:dict[str,Any],answer:Any)->GradeResult:
        ...
        if t in {'SINGLE_CHOICE','COMBOBOX_SELECT'} and contract.get('accepted_choice_ids'):
            correct=str(answer) in set(map(str,contract['accepted_choice_ids']));return GradeResult('CORRECT' if correct else 'INCORRECT',1.0 if correct else 0.0,node.get('success_feedback') if correct else node.get('failure_feedback'),**common)
        ...
        if t=='ORDERING' and contract.get('accepted_order'):
            correct=list(map(str,answer if isinstance(answer,list) else []))==list(map(str,contract['accepted_order']));return GradeResult('CORRECT' if correct else 'INCORRECT',1.0 if correct else 0.0,node.get('success_feedback') if correct else node.get('failure_feedback'),**common)
        if t=='MATCHING' and contract.get('accepted_pairs'):
            correct={str(k):str(v) for k,v in (answer or {}).items()}=={str(k):str(v) for k,v in contract['accepted_pairs'].items()};return GradeResult('CORRECT' if correct else 'INCORRECT',1.0 if correct else 0.0,node.get('success_feedback') if correct else node.get('failure_feedback'),**common)
        ...
        if t=='SHORT_TEXT':
            ...
            if got and got in candidates:return GradeResult('CORRECT',1.0,node.get('success_feedback',''),**common)
        ...
        if t in {'LONG_TEXT','ARGUMENT'}:return GradeResult('REVIEW_REQUIRED',None,'Аргумент збережено. Остаточне proposition/evidence grading надає DEV5 через GraderPort.',needs_human_or_dev5_grader=True,**common)
    ...
    @staticmethod
    def _evidence(node):
        v=node.get('required_evidence',[]);return list(map(str,v)) if isinstance(v,list) else ([str(v)] if v else [])
