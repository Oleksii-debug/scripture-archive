...
class RuntimeCompatTests(unittest.TestCase):
    ...
    def test_player_commands_map_to_current_dev5_runtime_v1(self):
        ...
        for public,internal in expected.items():
            rr=adapter.to_runtime_request(self.req(public,{"node_id":"LN01-N01","answer":"x"}))
            self.assertEqual("runtime.v1",rr["api_version"]);self.assertEqual(internal,rr["command"]);self.assertEqual("r-1",rr["request_id"])
    ...
    def test_runtime_response_must_be_json_safe_and_runtime_v1(self):
        ...
        response=adapter.invoke_runtime(self.req("player.load_node",{"node_id":"LN01-N01"}))
        self.assertEqual("LN01-N01",response["task"]["node_id"])
