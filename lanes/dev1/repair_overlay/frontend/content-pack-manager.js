...
function renderCandidates(data){
  ...
  inboxNode.textContent=data.inbox_path||'Недоступно';
}
...
function renderInstalled(data){
  ...
  const packs=Array.isArray(data.installed)?data.installed:[];
...
function renderSnapshot(data){renderCandidates(data);renderInstalled(data)}
function renderInspection(inspection){
  previewNode.replaceChildren();
  if(!inspection)return;
  const manifest=inspection.manifest||{};
  const dl=document.createElement('dl');
  [['Pack',`${manifest.pack_id||'—'} @ ${manifest.version||'—'}`],['Schema',manifest.content_schema_version||'—'],['Nodes',String(inspection.node_count??'—')],['Files',String(inspection.file_count??'—')],['SHA-256',inspection.archive_sha256||'—']].forEach(([key,value])=>{const dt=text('dt',key);const dd=text('dd',value);dl.append(dt,dd)});
  previewNode.append(dl);
}
async function refresh(){const data=await api('content_packs.list');if(data?.schema!=='scripture.content-pack-manager.v1')throw new Error('Некоректна відповідь Content Pack Manager.');renderSnapshot(data);return data}
async function runAction(command,payload,success){try{const data=await api(command,payload);renderSnapshot(data);renderInspection(data.inspection||null);announce(success)}catch(error){announce(`Помилка: ${error.message}`,true)}}
async function inspect(){try{const data=await api('content_packs.inspect',{file_name:selectedCandidate()});renderSnapshot(data);renderInspection(data.inspection);announce('Пакет перевірено без встановлення.')}catch(error){announce(`Помилка: ${error.message}`,true)}}
async function install(){try{const file_name=selectedCandidate();const data=await api('content_packs.install',{file_name,activate:false});renderSnapshot(data);renderInspection(data.inspection);announce(`Встановлено ${data.inspection?.manifest?.pack_id||file_name}; активація виконується окремо.`)}catch(error){announce(`Помилка: ${error.message}`,true)}}
